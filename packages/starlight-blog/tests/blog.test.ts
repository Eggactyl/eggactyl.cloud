...
test("should add a blog link to all pages with the configured title", async ({
  blogPage,
}) => {
  await blogPage.goto();

  await expect(
    blogPage.page.getByRole("link", { exact: true, name: "Blog" }),
  ).toBeVisible();
});
...
test("should not display the content panel containing the title", async ({
  blogPage,
}) => {
  await blogPage.goto();

  await expect(
    blogPage.page.getByRole("heading", { level: 1 }),
  ).not.toBeVisible();
});
...
test("should add links to recent posts in the sidebar", async ({
  blogPage,
}) => {
  await blogPage.goto();
...
test("should display a preview of each posts", async ({ blogPage }) => {
  ...
  await expect(
    articles.first().getByText("Donec eget vestibulum leo."),
  ).toBeVisible();
});
