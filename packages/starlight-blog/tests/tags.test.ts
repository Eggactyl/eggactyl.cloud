...
test("should display a preview of each posts with proper tag", async ({
  tagsPage,
}) => {
  const tag = "Starlight";
  const count = 4;
  ...
  await expect(
    tagsPage.page.getByText(`${count} posts with the tag “${tag}”`, {
      exact: true,
    }),
  ).toBeVisible();
  ...
  for (const article of await articles.all()) {
    expect(
      await article.getByRole("listitem").filter({ hasText: tag }).count(),
    ).toBe(1);
  }
});
