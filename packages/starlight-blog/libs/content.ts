  ...
  return entryPages.map((entries, index) => {
    ...
    return {
      ...
      props: {
        entries,
        nextLink: nextPage
          ? { href: `/releases/${index + 2}`, label: "Older posts" }
          : undefined,
        prevLink: prevPage
          ? {
              href: index === 1 ? "/releases" : `/releases/${index}`,
              label: "Newer posts",
            }
          : undefined,
...
export async function getBlogEntry(
  ...
  return {
    entry,
    nextLink: nextEntry
      ? { href: `/${nextEntry.slug}`, label: nextEntry.data.title }
      : undefined,
    prevLink: prevEntry
      ? { href: `/${prevEntry.slug}`, label: prevEntry.data.title }
      : undefined,
  };
}
...
export async function getBlogEntries() {
  const entries = await getCollection<StarlightEntryData>("docs", ({ id }) => {
    return id.startsWith("releases/") && id !== "releases/index.mdx";
  });
